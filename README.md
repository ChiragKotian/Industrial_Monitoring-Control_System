...
<p align="center">
<img width="600" alt="one node connected" src="https://github.com/user-attachments/assets/8abd4de7-f74c-4e3b-8f7e-fbe7e701a832" />
</p>
...
1. **Sub-GHz Penetration & EMI Resilience:** Standard Wi-Fi and Zigbee operate at 2.4 GHz. High-frequency waves easily bounce off the dense metal pipes, tanks, and blast walls of a refinery. Furthermore, 2.4 GHz is highly susceptible to the massive Electromagnetic Interference (EMI) generated by substation transformers. By utilizing LoRa in the **Sub-GHz band (865 MHz)**, the RF waves can bend around metal infrastructure and effortlessly penetrate thick concrete switchgear rooms without signal degradation.
2. **Asymmetric Bandwidth Optimization:** Wi-Fi was designed for high-throughput applications (like video streaming). AgnostiLink's Edge Gateways are designed to preprocess data locally and only transmit highly compressed, quantized telemetry strings (a few bytes per second). LoRa perfectly matches this requirement—sacrificing unnecessary bandwidth in exchange for extreme signal reliability.
3. **Star Topology vs. Mesh Overhead (CAPEX Reduction):** Competing OEM solutions (like the Schneider THT110) utilize Zigbee protocols. Zigbee has a maximum range of roughly 15 meters in industrial settings, requiring the facility to install dozens of expensive repeater nodes and secondary IT network switches to bounce the signal back to the server room. LoRa boasts a line-of-sight range of over **10 Kilometers**. A single gateway placed on a central control room roof can comfortably cover a massive multi-acre refinery plant, completely eliminating secondary network infrastructure.
...
| **Metal/Wall Penetration**| Poor | Poor | **Excellent (Sub-GHz)** |
...
* **Hardware Abstraction & Cross-Silicon Portability:** Because the firmware is built on top of the standard Arduino C++ Core (HAL), the LMP logic is not permanently locked to the Nano's silicon. If a technician encounters an isolated, highly congested switchgear panel where running physical Cat6 CAN cables is physically impossible, the exact same LMP code can be instantly compiled onto an ESP32. This enables rapid Wireless Bridging—for example, deploying a battery-powered ESP32 deep inside the inaccessible panel to read sensors, which then transmits the data via wireless ESP-NOW to a secondary ESP32 safely clipped to the wired CAN bus. This seamlessly extends the network into physically blocked zones without requiring any rewrites of the core CAN routing protocol.
...
📦 Industrial_Monitoring-Control_System
 ...
 ┃ ┣ 📂 V1
 ┃ ┃ ┣ 📂 Node_Main
 ┃ ┃ ┃ ┣ 📜 Node_Main.ino          # Main setup and RTOS task pinning
 ┃ ┃ ┃ ┣ 📜 Node_CAN.cpp           # AL-CAN protocol and assembly line logic
 ┃ ┃ ┃ ┣ 📜 Node_CAN.h
 ┃ ┃ ┃ ┣ 📜 Node_Config.h          # Set important variables
 ┃ ┃ ┃ ┣ 📜 Node_LoRa.cpp          # Encrypt and transmit data wirelessly
 ┃ ┃ ┃ ┣ 📜 Node_LoRa.h
 ┃ ┃ ┃ ┣ 📜 Node_Registry.cpp      # Store and handle LMP data, LMP register
 ┃ ┃ ┃ ┣ 📜 Node_Registry.h
 ┃ ┃ ┃ ┣ 📜 Node_Secrets.h         # Store AES key
 ┃ ┃ ┃ ┣ 📜 Node_Storage.cpp       # SD Card SPI routines and Deep-Copy Queue
 ┃ ┃ ┃ ┣ 📜 Node_Storage.h
 ┃ ┃ ┃ ┣ 📜 Node_UI.cpp            # State machine for the OLED display
 ┃ ┃ ┃ ┗ 📜 Node_UI.h          
 ┣ 📂 LMP        # Edge firmware for the Arduino Nanos
 ┃ ┣ 📂 V1
 ┃ ┃ ┣ 📂 LMP_code
 ┃ ┃ ┃ ┣ 📜 LMP_Code.ino           # Bare-metal non-blocking loop
 ┃ ┃ ┃ ┣ 📜 AHT20_driver.cpp       # Driver for AHT21 sensor
 ┃ ┃ ┃ ┣ 📜 AHT20_driver.h
...
**Our Solution:** We implemented **Dynamic SPI MISO Remapping**. Our firmware explicitly reconfigures the ESP32's MISO pin routing on-the-fly (`hspi.begin()`) whenever the storage task gains control of the bus. This software-defined isolation ensures 100% bus reliability regardless of the SD module's hardware limitations.
...
_Click the image below to watch the complete system demo._
<p align="center">
